...
class Block(nn.Module):
    ...
    def __init__(self, config):
        ...
        # Trainable logit for decay factor gamma (sigmoid(2.0) ≈ 0.88)
        self.gamma_logit = nn.Parameter(torch.tensor(2.0))

    def forward(self, x):
        # x = x + self.attn(self.ln_1(x))
        # x = x + self.mlp(self.ln_2(x))

        # 1. Attention calculates first on raw token input x
        x_attn = x + self.attn(self.ln_1(x))
        
        # 2. Learnable decay factor gamma strictly bounded in (0, 1)
        gamma = torch.sigmoid(self.gamma_logit)
        
        # 3. Build a numerically stable Causal Decay Matrix (T, T)
        T = x_attn.size(1)
        ...
        norm_weights = decay_weights / decay_weights.sum(dim=-1, keepdim=True)
        
        # 4. Apply decay matrix via matrix multiplication: (T, T) x (B, T, C) -> (B, T, C)
